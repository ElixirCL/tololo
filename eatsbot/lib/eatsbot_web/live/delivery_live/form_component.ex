defmodule EatsbotWeb.DeliveryLive.FormComponent do
  use EatsbotWeb, :live_component
  alias EatsbotWeb.DeliveryLive.LocationInputComponent
    ...
    <div>
      ...
      >
        <%= if @form.source.type == :create do %>
          ...
          <input hidden name="delivery[delivery_order][type]" value="text" />
          <.input
            type="map"
            name="delivery[delivery_order]"
            id="delivery_order"
            fields={[{"data", gettext("Name")}]}
  ...
  def update(assigns, socket) do
    ...
    possible_states =
      EatsbotCore.Deliveries.Transitions.get_possible_states(current_state) |> Enum.reverse()
  ...
  defp assign_form(%{assigns: %{delivery: delivery}} = socket) do
    form =
      if delivery do
        ...
        AshPhoenix.Form.for_create(EatsbotCore.Deliveries.Delivery, :initialize,
          as: "delivery",
          actor: @actor
        )
      end
