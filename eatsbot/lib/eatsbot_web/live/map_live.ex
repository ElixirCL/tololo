defmodule EatsbotWeb.MapLive do
  ...
  def mount(%{"token" => token}, _session, socket) do
    ...
    case get_delivery_via_token(token) do
      ...
      :error ->
        raise EatsbotWeb.NotFoundError, gettext("Delivery not found")
    end
  end

  def mount(_params, _session, _socket),
    do: raise(EatsbotWeb.NotFoundError, gettext("Token not found"))

  defp get_delivery_via_token(token) do
    case EatsbotCore.Deliveries.Delivery.get_via_token(token,
  ...
  defp date_to_string(date), do: Eatsbot.Cldr.Time.to_string(date) |> elem(1)
end
